...
- ログアウト
- Playwright E2E
  - 送信ウィザードのmultipart upload・shipment確定
  - 受信リンクのパスワード検証・Grant Cookie復元・download開始
  - Desktop Chrome相当／Pixel 7相当
...
`npm ci`は`package-lock.json`に記録された依存バージョンをそのままインストールします。依存パッケージを追加・更新する場合だけ`npm install <package>`を使用し、更新された`package.json`と`package-lock.json`を同じPRへ含めてください。
...
E2Eは本番AWS・Stripe・PostgreSQLへ接続せず、`e2e/mock-api.mjs`を利用してNext.jsのrewrite、multipart PUT、ETag、Access Grant Cookie、download eventを検証します。

初回だけChromiumとOS依存パッケージをインストールします。
...
画面を表示しながら確認する場合は以下を使用します。

```bash
npm run e2e:headed
```

失敗後のHTML reportを表示する場合は以下を使用します。
